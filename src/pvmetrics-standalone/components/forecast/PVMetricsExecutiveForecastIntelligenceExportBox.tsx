import { useState } from 'react';
import { PVMetricsExecutiveForecastIntelligenceSummary } from '../../types/pvmetrics-executive-forecast-intelligence.types';

type PVMetricsExecutiveForecastIntelligenceExportBoxProps = {
  summary: PVMetricsExecutiveForecastIntelligenceSummary;
};

export const PVMetricsExecutiveForecastIntelligenceExportBox = ({
  summary,
}: PVMetricsExecutiveForecastIntelligenceExportBoxProps) => {
  const [copiedTarget, setCopiedTarget] = useState<
    'internal' | 'client' | null
  >(null);

  const handleCopy = async (
    target: 'internal' | 'client',
    value: string,
  ) => {
    try {
      await navigator.clipboard.writeText(value);
      setCopiedTarget(target);
      window.setTimeout(() => setCopiedTarget(null), 1800);
    } catch {
      setCopiedTarget(null);
    }
  };

  return (
    <section className="rounded-3xl border border-slate-700/70 bg-slate-950/70 p-5 shadow-2xl">
      <div className="mb-4">
        <p className="text-xs font-semibold uppercase tracking-[0.3em] text-fuchsia-300">
          EXECUTIVE FORECAST INTELLIGENCE EXPORT
        </p>

        <h3 className="text-xl font-bold text-slate-50">
          Textos ejecutivos copiables
        </h3>

        <p className="mt-2 text-sm text-slate-400">
          Panel local para copiar reporte interno ejecutivo y resumen cliente.
          No envía correos, no exporta PDF y no usa datos reales.
        </p>
      </div>

      <div className="grid gap-4 xl:grid-cols-2">
        <article className="rounded-2xl border border-emerald-400/20 bg-emerald-950/10 p-4">
          <div className="flex items-center justify-between gap-3">
            <h4 className="font-semibold text-slate-100">
              Reporte interno ejecutivo
            </h4>

            <button
              type="button"
              onClick={() =>
                handleCopy('internal', summary.internalExecutiveText)
              }
              className="rounded-xl border border-emerald-400/40 px-3 py-2 text-xs font-semibold uppercase tracking-wide text-emerald-100 hover:bg-emerald-400/10"
            >
              {copiedTarget === 'internal' ? 'Copiado' : 'Copiar'}
            </button>
          </div>

          <textarea
            readOnly
            value={summary.internalExecutiveText}
            className="mt-3 h-96 w-full rounded-2xl border border-slate-800 bg-slate-950 p-3 font-mono text-xs text-slate-300"
          />
        </article>

        <article className="rounded-2xl border border-fuchsia-400/20 bg-fuchsia-950/10 p-4">
          <div className="flex items-center justify-between gap-3">
            <h4 className="font-semibold text-slate-100">
              Resumen cliente ejecutivo
            </h4>

            <button
              type="button"
              onClick={() => handleCopy('client', summary.clientExecutiveText)}
              className="rounded-xl border border-fuchsia-400/40 px-3 py-2 text-xs font-semibold uppercase tracking-wide text-fuchsia-100 hover:bg-fuchsia-400/10"
            >
              {copiedTarget === 'client' ? 'Copiado' : 'Copiar'}
            </button>
          </div>

          <textarea
            readOnly
            value={summary.clientExecutiveText}
            className="mt-3 h-96 w-full rounded-2xl border border-slate-800 bg-slate-950 p-3 font-mono text-xs text-slate-300"
          />
        </article>
      </div>

      <div className="mt-4 rounded-2xl border border-amber-400/20 bg-amber-950/20 p-4">
        <p className="text-xs font-semibold uppercase tracking-wide text-amber-300">
          Nota ejecutiva
        </p>

        <p className="mt-2 text-sm text-amber-100">
          Este resumen es mock y no contractual. No representa forecast real,
          reporte CEN, lectura SCADA, facturación, ingreso ni pérdida real.
        </p>
      </div>
    </section>
  );
};
