  ...
  if (selectedPlant && selectedSummary) {
    ...
    return (
      <div className="space-y-5">
        ...
        <Panel>
          <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
            ...
            <dl className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              <div className="rounded-xl bg-slate-950/70 p-3"><dt className="text-[9px] uppercase text-slate-500">{t.capacity}</dt><dd className="mt-1 font-mono text-sm font-bold text-white"><AvailabilityValue value={selectedSummary.nominalCapacity} locale={locale} t={t} /></dd></div>
              <div className="rounded-xl bg-slate-950/70 p-3"><dt className="text-[9px] uppercase text-slate-500">{t.region}</dt><dd className="mt-1 text-sm font-bold text-white">{selectedPlant.operatingRegion}</dd></div>
              <div className="rounded-xl bg-slate-950/70 p-3"><dt className="text-[9px] uppercase text-slate-500">{t.cases}</dt><dd className="mt-1 font-mono text-sm font-bold text-white">{selectedSummary.caseCount}</dd></div>
              <div className="rounded-xl bg-slate-950/70 p-3"><dt className="text-[9px] uppercase text-slate-500">{t.opportunityScore}</dt><dd className="mt-1 font-mono text-sm font-bold text-white" title={t.scoreTooltip}>{selectedSummary.climateOpportunityScore.score.toFixed(2)}</dd></div>
            </dl>
          </div>
        </Panel>
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-4">
          <Panel className="lg:col-span-2">
            ...
            <dl className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-3"><dt className="text-[10px] uppercase text-slate-500">{t.recoverableEnergy}</dt><dd className="mt-2 font-mono text-lg font-bold text-white"><AvailabilityValue value={selectedSummary.estimatedRecoverableEnergy} locale={locale} t={t} /></dd></div>
              <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-3"><dt className="text-[10px] uppercase text-slate-500">{t.climateImpact}</dt><dd className="mt-2 font-mono text-lg font-bold text-white"><AvailabilityValue value={selectedSummary.estimatedClimateImpact} locale={locale} t={t} /></dd></div>
            </dl>
          </Panel>
        ...
        <Panel><h3 className="flex items-center gap-2 text-sm font-bold text-amber-300"><TriangleAlert className="h-4 w-4" />{t.detailsAndLimitations}</h3><div className="mt-3 grid grid-cols-1 gap-4 lg:grid-cols-2"><div><h4 className="text-[10px] font-bold uppercase text-slate-500">{t.warnings}</h4>{selectedSummary.warnings.length ? <ul className="mt-2 list-disc space-y-1 pl-5 text-xs leading-5 text-slate-300">{selectedSummary.warnings.map((warning) => <li key={warning}>{warning}</li>)}</ul> : <p className="mt-2 text-xs text-slate-500">{t.noneDocumented}</p>}</div><div><h4 className="text-[10px] font-bold uppercase text-slate-500">{t.limitations}</h4><ul className="mt-2 list-disc space-y-1 pl-5 text-xs leading-5 text-slate-300">{[...selectedPlant.limitations, ...selectedSummary.limitations].map((limitation, index) => <li key={`${index}:${limitation}`}>{limitation}</li>)}</ul></div></div></Panel>
        <section>
          <SectionHeader title={t.opportunities} action={<label className="text-[10px] font-bold uppercase text-slate-400">{t.recoverability}<select aria-label={`${t.recoverability}: ${selectedPlant.name}`} value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="ml-2 min-h-10 rounded-lg border border-slate-700 bg-slate-950 px-3 text-xs text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-400"><option value="">{t.all}</option><option value="recoverable">{t.statusLabels.recoverable}</option><option value="partially-recoverable">{t.statusLabels['partially-recoverable']}</option><option value="non-recoverable">{t.statusLabels['non-recoverable']}</option><option value="indeterminate">{t.statusLabels.indeterminate}</option></select></label>} />
          <div className="mt-4 space-y-3">
            {filteredCases.length === 0 ? <EmptyState title={t.noResults} /> : filteredCases.map((item) => (
              <article key={item.caseId} className="flex flex-col justify-between gap-3 rounded-xl border border-slate-800 bg-slate-900/60 p-4 md:flex-row md:items-center">
                <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><StatusBadge value={item.priority.value?.toString() ?? 'medium'} label={item.priority.formattedValue} t={t} /><StatusBadge value={item.recoverability} t={t} /></div><h3 className="mt-2 truncate text-sm font-bold text-white">{item.title}</h3><p className="mt-1 text-[10px] text-slate-500">{item.category} · {item.recommendedNextStep}</p></div>
        ...
        {plants.map((plant) => {
          ...
          return (
            <article key={plant.id} className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
              ...
              <h3 className="mt-4 text-lg font-black text-white">{plant.name}</h3><p className="mt-1 text-[11px] text-slate-500">{plant.assetType} · {plant.operatingRegion}</p>
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 text-[10px]"><p className="rounded-lg bg-slate-950/60 p-2 text-slate-400">{t.highPriority}: <strong className="text-white">{summary.highPriorityCaseCount}</strong></p><p className="rounded-lg bg-slate-950/60 p-2 text-slate-400">{t.pendingReview}: <strong className="text-white">{summary.pendingReviewCount}</strong></p>{summary.featuredCaseId && <p className="col-span-2 rounded-lg bg-slate-950/60 p-2 text-slate-400">{t.featuredCase}: <strong className="font-mono text-white">{summary.featuredCaseId}</strong></p>}</div>
              {summary.overlapWarningCount > 0 && <p className="mt-3 flex gap-2 text-[10px] text-amber-300"><TriangleAlert className="h-3.5 w-3.5" />{t.overlapPenalty}: −{summary.overlapPenalty}</p>}
