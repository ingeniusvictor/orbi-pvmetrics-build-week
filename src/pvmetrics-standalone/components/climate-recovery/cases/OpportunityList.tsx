    ...
    <div className="space-y-5">
      ...
      <section aria-label={t.filters} className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
        <div className="grid grid-cols-1 gap-3 min-[430px]:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-6">
          <label className="text-[10px] font-bold uppercase text-slate-400">{t.plant}<select aria-label={t.plant} className={`${controlClass} mt-1`} value={filters.plantId} onChange={(event) => update('plantId', event.target.value)}><option value="">{t.all}</option>{plants.map((plant) => <option key={plant.id} value={plant.id}>{plant.name}</option>)}</select></label>
          <label className="text-[10px] font-bold uppercase text-slate-400">{t.category}<select aria-label={t.category} className={`${controlClass} mt-1`} value={filters.category} onChange={(event) => update('category', event.target.value)}><option value="">{t.all}</option>{options(availableFilters.category, t)}</select></label>
          <label className="text-[10px] font-bold uppercase text-slate-400">{t.recoverability}<select aria-label={t.recoverability} className={`${controlClass} mt-1`} value={filters.recoverability} onChange={(event) => update('recoverability', event.target.value)}><option value="">{t.all}</option>{options(availableFilters.recoverability, t)}</select></label>
          <label className="text-[10px] font-bold uppercase text-slate-400">{t.priority}<select aria-label={t.priority} className={`${controlClass} mt-1`} value={filters.priorityBand} onChange={(event) => update('priorityBand', event.target.value)}><option value="">{t.all}</option>{options(availableFilters.priorityBand, t)}</select></label>
          <label className="text-[10px] font-bold uppercase text-slate-400">{t.confidence}<select aria-label={t.confidence} className={`${controlClass} mt-1`} value={filters.confidenceLevel} onChange={(event) => update('confidenceLevel', event.target.value)}><option value="">{t.all}</option>{options(availableFilters.confidenceLevel, t)}</select></label>
          <label className="text-[10px] font-bold uppercase text-slate-400">{t.dataSufficiency}<select aria-label={t.dataSufficiency} className={`${controlClass} mt-1`} value={filters.dataSufficiency} onChange={(event) => update('dataSufficiency', event.target.value)}><option value="">{t.all}</option>{options(availableFilters.dataSufficiency, t)}</select></label>
          <label className="text-[10px] font-bold uppercase text-slate-400">{t.review}<select aria-label={t.review} className={`${controlClass} mt-1`} value={filters.humanReviewRequired} onChange={(event) => update('humanReviewRequired', event.target.value)}><option value="">{t.all}</option><option value="true">{t.required}</option><option value="false">{t.notRequired}</option></select></label>
          <label className="text-[10px] font-bold uppercase text-slate-400">{t.climateAvailability}<select aria-label={t.climateAvailability} className={`${controlClass} mt-1`} value={filters.climateImpactAvailability} onChange={(event) => update('climateImpactAvailability', event.target.value)}><option value="">{t.all}</option>{options(availableFilters.climateImpactAvailability, t)}</select></label>
          <label className="text-[10px] font-bold uppercase text-slate-400">{t.sort}<select aria-label={`${t.sort}: ${t.opportunities}`} className={`${controlClass} mt-1`} value={sort} onChange={(event) => setSort(event.target.value as ClimateRecoveryCatalogSort)}><option value="priority-desc">{t.highPriority} ↓</option><option value="recoverable-energy-desc">{t.estimatedEnergy} ↓</option><option value="climate-impact-desc">{t.climateImpact} ↓</option><option value="confidence-desc">{t.confidence} ↓</option><option value="case-title-asc">{t.case} A–Z</option></select></label>
        ...
        <>
          <div className="hidden overflow-x-auto rounded-2xl border border-slate-800 xl:block">
            <table className="w-full min-w-[1200px] border-collapse text-left text-xs">
              <thead className="bg-slate-950/80 text-[9px] uppercase tracking-wide text-slate-500"><tr>{[t.case, t.plant, t.category, t.recoverability, t.priority, t.confidence, t.dataSufficiency, t.estimatedEnergy, t.climateImpact, t.review, t.recommendedAction, ''].map((label) => <th key={label} scope="col" className="px-3 py-3">{label}</th>)}</tr></thead>
              <tbody className="divide-y divide-slate-800">{cases.map((item) => (
                <tr key={item.caseId} className="bg-slate-900/40 hover:bg-slate-800/40">
                  <td className="max-w-52 px-3 py-3"><div className="font-bold text-white">{item.title}</div><div className="mt-1 font-mono text-[9px] text-slate-500">{item.caseId}</div></td>
                  ...
                  <td className="px-3 py-3">{item.humanReviewRequired ? <StatusBadge value="pending-review" label={t.required} t={t} /> : <StatusBadge value="low" label={t.notRequired} t={t} />}</td><td className="max-w-56 px-3 py-3 text-[10px] leading-4 text-slate-400">{item.recommendedNextStep}</td>
          ...
          <div className="grid grid-cols-1 gap-3 xl:hidden">{cases.map((item) => (
            <article key={item.caseId} className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
              ...
              <h3 className="mt-3 text-sm font-bold text-white">{item.title}</h3><p className="mt-1 text-[10px] text-slate-500">{item.plantName} · {item.category}</p>
              <dl className="mt-4 grid grid-cols-2 gap-2 text-[10px]"><div className="rounded-lg bg-slate-950/60 p-2"><dt className="text-slate-500">{t.priority}</dt><dd className="mt-1 font-mono text-slate-200"><AvailabilityValue value={item.priority} locale={locale} t={t} /></dd></div><div className="rounded-lg bg-slate-950/60 p-2"><dt className="text-slate-500">{t.confidence}</dt><dd className="mt-1 font-mono text-slate-200"><AvailabilityValue value={item.confidence} locale={locale} t={t} /></dd></div><div className="rounded-lg bg-slate-950/60 p-2"><dt className="text-slate-500">{t.estimatedEnergy}</dt><dd className="mt-1 font-mono text-slate-200"><AvailabilityValue value={item.estimatedRecoverableEnergy} locale={locale} t={t} /></dd></div><div className="rounded-lg bg-slate-950/60 p-2"><dt className="text-slate-500">{t.climateImpact}</dt><dd className="mt-1 font-mono text-slate-200"><AvailabilityValue value={item.estimatedClimateImpact} locale={locale} t={t} /></dd></div></dl>
              <p className="mt-3 text-[10px] leading-4 text-slate-400">{item.recommendedNextStep}</p><button type="button" onClick={() => onCase(item.caseId)} className={`${buttonClass} mt-4 w-full`}>{t.inspectCase}<ArrowRight className="h-4 w-4" /></button>
            </article>
          ))}</div>
        </>
      )}
