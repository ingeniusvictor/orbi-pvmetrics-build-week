...
const ClimateRecoveryView: React.FC<{
  ...
  const navigationResult = useGuidedDemoNavigation({
    active: guidedState.active,
    step: currentStep,
    reducedMotion: guidedState.reducedMotion,
    prepareStep: prepareGuidedStep,
  });
  ...
  const startGuided = useCallback(() => {
    if (!guidedState.active && rootRef.current) {
      contextBeforeGuidedDemo.current = captureGuidedDemoContext(
        rootRef.current,
        demo.activeSection,
        demo.selectedPlantId,
        demo.selectedCaseId,
      );
    }
  ...
  const move = useCallback((direction: 'next' | 'previous' | 'skip') => {
    setGuidedState((state) => {
      const nextState = direction === 'previous'
        ? previousGuidedDemo(state)
        : direction === 'skip'
          ? skipGuidedDemo(state)
          : advanceGuidedDemo(state);
      return { ...nextState, narrationVisible: true };
    });
  }, []);

  const toggleNarrative = useCallback(() => {
    setGuidedState((state) => ({ ...state, narrationVisible: !state.narrationVisible }));
  }, []);
  ...
  const guidedAnchor = guidedState.active ? {
    id: currentStep.anchorId,
    stepId: currentStep.id,
    label: `${t.guidedDemo}: ${t.step} ${currentStep.order}`,
  } : undefined;
  const caseGuidedAnchor = currentStep.caseId === demo.selectedCaseId ? guidedAnchor : undefined;
  const navigationMessage = navigationResult && navigationResult.status !== 'success'
    ? t.guidedTargetUnavailable
    : undefined;
  ...
  const workspace = (
    <div className="min-w-0 space-y-5">
      ...
      <nav aria-label="Climate Recovery sections" className={`cr-mobile-tabs z-20 -mx-1 overflow-x-auto rounded-2xl border border-slate-800 bg-slate-950/95 p-2 shadow-xl shadow-slate-950/40 backdrop-blur ${guidedState.active ? 'lg:sticky lg:top-0' : 'sticky top-0'}`}>
      ...
      <GuidedDemoHighlight active={guidedState.active}>
        <main className="min-w-0" data-guided-mode={guidedState.active ? 'guided' : 'free'}>
          ...
          {demo.activeSection === 'review' && <HumanReviewQueue queue={demo.executive.reviewQueue} plants={demo.plants} caseTitle={demo.caseTitle} t={t} locale={demo.locale} onCase={demo.selectCase} guidedAnchor={currentStep.id === 'explainability' ? guidedAnchor : undefined} />}
          {demo.activeSection === 'case' && <Suspense fallback={<div className="rounded-2xl border border-cyan-500/20 bg-slate-900 p-5 text-xs text-cyan-200">{t.caseSummary}</div>}><CaseDetailView key={`${demo.selectedCaseId}:${currentStep.id}:${guidedState.active}`} detail={demo.selectedCase} locale={demo.locale} t={t} onBack={() => demo.navigate('opportunities')} guidedOpenSections={guidedState.active ? currentStep.requiredOpenSections : undefined} guidedAnchor={caseGuidedAnchor} /></Suspense>}
        </main>
      </GuidedDemoHighlight>
    ...
    >
      ...
      {guidedState.active ? (
        <div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(320px,420px)_minmax(0,1fr)] lg:items-start">
          <Suspense fallback={<div className="rounded-2xl border border-amber-500/20 bg-slate-900 p-4 text-xs text-amber-200">{t.guidedDemo}</div>}>
            <GuidedDemoShell step={currentStep} locale={demo.locale} t={t} onPrevious={() => move('previous')} onNext={next} onSkip={() => move('skip')} onExit={() => exitGuided(true)} onReset={resetDemo} narrativeExpanded={guidedState.narrationVisible} onToggleNarrative={toggleNarrative} navigationMessage={navigationMessage} />
          </Suspense>
          {workspace}
        </div>
      ) : workspace}
