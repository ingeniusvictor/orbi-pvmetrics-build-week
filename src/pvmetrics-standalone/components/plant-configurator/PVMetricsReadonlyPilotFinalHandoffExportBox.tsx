...
type PVMetricsReadonlyPilotFinalHandoffExportBoxProps = {
  handoff: PVMetricsReadonlyPilotFinalHandoffPackage;
};

export const PVMetricsReadonlyPilotFinalHandoffExportBox = ({
  handoff,
}: PVMetricsReadonlyPilotFinalHandoffExportBoxProps) => {
    ...
    <section className="rounded-3xl border border-slate-700/70 bg-slate-950/70 p-5 shadow-2xl">
      <div className="mb-4">
        ...
        <h3 className="text-xl font-bold text-slate-50">
          Textos copiables del handoff final
        </h3>
        <p className="mt-2 text-sm text-slate-400">
          Panel local para copiar el resumen interno o el texto de handoff cliente. No envía correos, no guarda y no exporta archivos.
        </p>
      </div>
        ...
        <article className="rounded-2xl border border-emerald-400/20 bg-emerald-950/10 p-4">
          <div className="flex items-center justify-between gap-3">
            ...
            <button
              type="button"
              onClick={() =>
                handleCopy('internal', handoff.internalHandoffText)
              }
          ...
          <textarea
            readOnly
            value={handoff.internalHandoffText}
        ...
        <article className="rounded-2xl border border-cyan-400/20 bg-cyan-950/10 p-4">
          <div className="flex items-center justify-between gap-3">
            ...
            <button
              type="button"
              onClick={() =>
                handleCopy('client', handoff.clientHandoffText)
              }
          ...
          <textarea
            readOnly
            value={handoff.clientHandoffText}
            className="mt-3 h-96 w-full rounded-2xl border border-slate-800 bg-slate-950 p-3 font-mono text-xs text-slate-300"
          />
        </article>
      </div>
      ...
      <div className="mt-4 rounded-2xl border border-amber-400/20 bg-amber-950/20 p-4">
        ...
        <p className="mt-2 text-sm text-amber-100">
          Este handoff es conceptual y local. No autoriza conexión real, no reemplaza revisión humana y no habilita integración productiva.
        </p>
      </div>
    </section>
  );
